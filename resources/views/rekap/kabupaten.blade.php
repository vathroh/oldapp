...
<table class="table table-striped table-dark table-bordered mt-3">
    ...
    <tbody>
        ...
        <tr>
            ...
            <td>{{$documents->where('kode_kab', $kab->kode_kab)->where('jenis_dokumen', 'DOKUMENTASI MP2K')->count()}} </td>
            <td>{{$documents->where('kode_kab', $kab->kode_kab)->where('jenis_dokumen', 'DOKUMENTASI OJT')->count()}} </td>
            <td>{{$documents->where('kode_kab', $kab->kode_kab)->where('jenis_dokumen', 'DOKUMENTASI PELATIHAN')->count()}} </td>
